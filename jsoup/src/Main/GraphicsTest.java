...
public class GraphicsTest {
	...
	public GraphicsDevice gd = GraphicsEnvironment
	...
	private String getDeviceConfigurationString(GraphicsConfiguration gc) {
		return "Bounds: "
				...
				+ "   -Is Page Flipping: "
				+ gc.getBufferCapabilities().isPageFlipping() + "\n"
				+ "Device: " + gc.getDevice() + "\n"
				+ "   -Available Accelerated Memory: "
				+ gc.getDevice().getAvailableAcceleratedMemory() + "\n"
	...
	class AnimationThread extends Thread {
		...
		/*
		 * public void setFullscreen(){ fullscreen = true; run = false;
		 * 
		 * frame.dispose(); frame = new JFrame(); frame.setUndecorated(true);
		 * 
		 * gd.setFullScreenWindow(frame);
		 * 
		 * frame.createBufferStrategy(3); bufferStrategy =
		 * frame.getBufferStrategy(); bufferCapabilities =
		 * gc.getBufferCapabilities();
		 * 
		 * run = true; new AnimationThread().start(); } public void setNormal(){
		 * if (fullscreen) { y = 0; fullscreen = false; run = false;
		 * 
		 * frame.dispose(); frame = new JFrame();
		 * frame.setSize(d.getGameWidth(), d.getGameHeight());
		 * frame.setLocationRelativeTo(null);
		 * frame.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE); input
		 * = new InputHandler(); frame.addKeyListener(input);
		 * frame.addFocusListener(input); frame.addMouseListener(input);
		 * frame.addMouseMotionListener(input);
		 * frame.addMouseWheelListener(input); frame.setUndecorated(true);
		 * frame.setAlwaysOnTop(true); frame.setVisible(true);
		 * 
		 * frame.createBufferStrategy(3); bufferStrategy =
		 * frame.getBufferStrategy(); bufferCapabilities =
		 * gc.getBufferCapabilities(); acc =
		 * gc.getBufferCapabilities().getBackBufferCapabilities
		 * ().isAccelerated(); d.fullscreen = false;
		 * 
	...
	public void draw(Graphics2D g2) {
		...
		g2.setFont(font);
		/*
		 * g2.setColor(Color.black); g2.fillRect(0, 0, frame.getWidth(),
		 * frame.getHeight());
		 * 
		 * g2.setColor((bufferCapabilities.isPageFlipping() &&
		 * bufferCapabilities .isMultiBufferAvailable()) ? Color.white :
		 * errorColor); g2.drawString( (bufferCapabilities.isPageFlipping() &&
		 ...
		 * errorColor); g2.drawString("Page Flipping: " +
		 ...
		 * Color.white); g2.drawString( "Full Screen Required: " +
		 ...
		 * : errorColor); g2.drawString( "Multiple Buffer Capable: " +
		 * (bufferCapabilities.isMultiBufferAvailable() ? "Yes" : "No"), 100,
		 * 120);
		 * 
		 * g2.setColor(gd.isFullScreenSupported() ? Color.white : errorColor);
		 * g2.drawString("Full-Screen Supported: " + (gd.isFullScreenSupported()
		 * ? "Yes" : "No"), 100, 140);
		 */
		g2.setColor((gc.getBufferCapabilities().getBackBufferCapabilities()
				.isAccelerated()
				&& gc.getBufferCapabilities().getFrontBufferCapabilities()
						.isAccelerated() && gc.getImageCapabilities()
				.isAccelerated()) ? Color.white : errorColor);
		g2.drawString(
				"BackBuffer Accelerated:  "
						+ ((gc.getBufferCapabilities()
								.getBackBufferCapabilities().isAccelerated()) ? "Yes"
								: "No"), 51, 160);
		g2.drawString(
				"FrontBuffer Accelerated:  "
						+ ((gc.getBufferCapabilities()
								.getFrontBufferCapabilities().isAccelerated()) ? "Yes"
								: "No"), 50, 170);
		g2.drawString(
				"ImageBuffer Accelerated: "
						+ ((gc.getImageCapabilities().isAccelerated()) ? "Yes"
								: "No"), 50, 180);
		g2.drawString("Memory: " + gd.getAvailableAcceleratedMemory(), 50, 190);

		if (gd.isFullScreenSupported()) {
			g2.setColor(Color.gray);
			g2.drawString(fullscreen ? "Press ESC to exit full-screen mode."
					: "F12 to enter Full-Screen Exclusive mode.", 50, 220);
		} else {
			g2.setColor(new Color(173, 0, 8));
			g2.drawString("Full-screen exclusive mode is not supported...", 50,
					220);
		}
		...
		/*
		 * y += delta; if ((y + 50) > frame.getHeight() || y < 0) { delta *= -1;
		 * }
		 * 
		 * g2.setColor(Color.blue); g2.fillRect(frame.getWidth() - 50, y, 50,
		 * 50);
		 */
	}
}
